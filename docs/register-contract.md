...
This code goes back in `quickstart.ts` — the Node/TypeScript project from Quickstart — not in the `z-tenant-flight` Rust repo. Append it to the bottom, after the `TenantClient` code from [Set Up Dev Env](/developers/adk/get-started/prerequisites/set-up-dev-env), using the `cat >> quickstart.ts << 'EOF'` pattern from [Running the code samples in this guide](/developers/adk/tips/running-code-samples).
