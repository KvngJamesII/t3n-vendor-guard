...
<Steps>
  ...
  <Step title="Build a TenantClient from your session">
    ...
    Append this to the bottom of the same `quickstart.ts` you created in Quickstart — it needs `t3n` and `tenantDid` from that file's scope. See [Running the code samples in this guide](/developers/adk/tips/running-code-samples) for the `cat >> quickstart.ts << 'EOF'` pattern:
    ...
    <Note>
      **`me()` lives on `tenant.tenant`, not `tenant` directly.** `TenantClient` exposes `tenant`/`maps`/`contracts`/`token` as sub-namespaces — `me()` and `claim()` are on the `tenant` sub-namespace (`TenantNamespace`), so the call is `tenant.tenant.me()`.
    </Note>
